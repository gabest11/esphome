...
class RcSwitchButBetterBinarySensor : public RemoteReceiverBinarySensorBase {
 public:
  void set_protocol(const RcSwitchButBetterProtocol &protocol) {
 ...
 protected:
  ...
  bool matches(RemoteReceiveData src) override {
    auto res = this->protocol_.decode(src);
    if (!res.has_value() || res->size() != this->code_.size()) {
      return false;
    }
    for (size_t i = 0; i < this->code_.size(); i++) {
      if ((this->code_[i] == '0' || this->code_[i] == '1') && this->code_[i] != (*res)[i]) {
        return false;
      }
    }
    return true;
